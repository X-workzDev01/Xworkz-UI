...
const Attandance = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            renderCell: (params) => ( <
                ...
                <
                Button onClick = {
                    () => handleButtonClickYes(params.row)
                }
                disabled = {
                    params.row.isButton || clickedButtonIds.has(params.row.id)
                }
                style = {
                    { color: params.row.ycolor }
                }
                variant = "outlined" >
                Yes <
                /Button> <
                Button onClick = {
                    () => handleButtonClickNo(params.row)
                }
                disabled = {
                    params.row.isButton || clickedButtonIds.has(params.row.id)
                }
                style = {
                    { color: params.row.ncolor }
                }
                variant = "outlined" >
                No <
                /Button> < /
    ...
    return ( <
        ...
        style = {
            { marginTop: "50px", display: "flex", alignItems: "center" }
        } >
        <
        FormControl >
        <
        ...
        rows = { gridData.rows }
        pagination autoHeight paginationModel = { paginationModel }
        pageSizeOptions = {
            [5, 10, 15, 20]
        }
