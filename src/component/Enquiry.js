...
function Enquiry() {
	...
	const checkPhoneNumberExists = async phoneNumber => {
		try {
			const response = await axios.get(
				Urlconstant.url + `api/contactNumberCheck?contactNumber=${phoneNumber}`,
				{
					headers: setCommonHeaders()
				}
			);
	...
	const checkEmailExists = async email => {
		try {
			const response = await axios.get(
				Urlconstant.url + `api/emailCheck?email=${email}`,
				{
					headers: setCommonHeaders()
				}
			);
				...
				<TextField
					...
					helperText={
						errors.email || emailExists
							? emailExists ? "Email already exists" : "Invalid Email Address"
							: ""
					}
				/>
				<TextField
