...
const Absentees = () => {
  ...
  const handleBatchChange = (event) => {
    ...
    axios
      .get(Urlconstant.url + `api/attendance/trainee?batch=${selectedBatchValue}`)
      .then((response) => {
  ...
  const handleAttendanceSubmit = () => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      axios.post(Urlconstant.url + `api/attendance/absentees?batch=${selectedBatch}`, attendanceData)
      .then((response) => {
  ...
  return (
    <div style={styles.container}>
      <div style={styles.form}>
        ...
        <Button variant="contained" onClick={handleAttendanceSubmit} disabled={isSubmitDisabled}>
          Submit Attendance
        </Button>
      </div>
    </div>
