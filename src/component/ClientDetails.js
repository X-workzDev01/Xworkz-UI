...
export default function ClientDetails() {
	...
	const handleSubmit = e => {
		...
		try {
			...
			axios
				.post(Urlconstant.url + "api/registerclient", clientData)
				.then(response => {
					setOpen(true);
	...
	const handleCompanyWebsite = event => {
		...
		if (website.trim() !== "" && validateWebsite(website)) {
			axios
				.get(
					Urlconstant.url + `/api/checkCompanyWebsite?companyWebsite=${website}`
				)
				.then(res => {
					...
					<Grid container spacing={3}>
						...
						<Grid item xs={12} sm={4}>
							<TextField
								...
								value={
									formData.sourceOfConnection ? formData.sourceOfConnection : ""
								}
								onChange={handleChange}
								fullWidth
								margin="normal"
