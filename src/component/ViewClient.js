...
function searchServerRows(searchValue, callBackDate, clientType) {
	const apiUrl =
		Urlconstant.url +
		`api/getdetailsbycompanyname?companyName=${searchValue}&callBackDate=${callBackDate}&clientType=${clientType}`;
	const requestOptions = {
		method: "GET"
	};
...
async function fetchFilteredData(searchValue, callBackDate, clientType) {
	try {
		const apiUrl =
			Urlconstant.url +
			`api/client/suggestions?companyName=${searchValue}&callBackDate=${callBackDate}&clientType=${clientType}`;
		const requestOptions = {
			method: "GET"
		};
					...
					slots={{
						toolbar: () =>
							<GridToolbarContainer>
								<Button onClick={handleColumnChange}>
									{" "}<MoreVert /> Columns
								</Button>
